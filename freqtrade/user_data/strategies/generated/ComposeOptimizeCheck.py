...
import freqtrade.vendor.qtpylib.indicators as qtpylib
import pandas as pd
class ComposeOptimizeCheck(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = '5m'
    can_short = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 数学条件: (EMA20_t > EMA60_t) ∧ (EMA20_{t-1} ≤ EMA60_{t-1}) ∧ (RSI_t < 70)
        dataframe.loc[
            (
        ...
        # 数学条件: (EMA20_t < EMA60_t) ∧ (EMA20_{t-1} ≥ EMA60_{t-1}) ∧ (RSI_t > 30)
        dataframe.loc[
            (
                (dataframe['ema_cross'] == -1) &
                (dataframe['ema_cross'].shift(1) == 1) &
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 多头离场条件：EMA20下穿EMA60
        # 数学条件: (EMA20_t < EMA60_t) ∧ (EMA20_{t-1} ≥ EMA60_{t-1})
        dataframe.loc[
            (
                (dataframe['ema_cross'] == -1) &
                (dataframe['ema_cross'].shift(1) == 1)
    ...
    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
        ...
        """
        将总仓位资金分为3等份，用于分批入场。
        数学原理: 仓位分割为S/3，其中S为总分配资金，实现资金管理
        """
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        数学原理:
        1. 加仓: 当P ≤ -0.03时，增加ΔS = S/3的仓位，其中P为当前收益率
        2. 减仓: 当P ≥ 0.05时，减少ΔQ = 0.5Q的持仓量，其中Q为当前持仓数量
        """
